...
def create_nuscenes_infos(root_path,
    ...
    if version == 'v1.0-trainval':
        import json
        with open("/home/aiyer43/UniV2X/data/split_datas/cooperative-split-data-spd.json",'r') as f:
            data = json.load(f)
        
        train_scenes = data['batch_split']['train']#splits.train
        val_scenes = data['batch_split']['val']#splits.val
        #print(val_scenes)
...
def get_available_scenes(nusc):
    ...
    for scene in nusc.scene:
        ...
        sd_rec = nusc.get('sample_data', sample_rec['data']['INFRASTRUCTURE_LIDAR_V1'])
...
def _get_can_bus_info(nusc, sample):
    return np.zeros(18)
...
def _fill_trainval_infos(nusc,
    ...
    for sample in mmcv.track_iter_progress(nusc.sample):
        ...
        lidar_token = sample['data']['INFRASTRUCTURE_LIDAR_V1']
        sd_rec = nusc.get('sample_data', sample['data']['INFRASTRUCTURE_LIDAR_V1'])
        cs_record = nusc.get('calibrated_sensor',
                             sd_rec['calibrated_sensor_token'])
        pose_record = nusc.get('ego_pose', sd_rec['ego_pose_token'])
        if sample['prev'] != '':
            sample_prev = nusc.get('sample', sample['prev'])
            sd_rec_prev = nusc.get('sample_data', sample_prev['data']['INFRASTRUCTURE_LIDAR_V1'])
        ...
        if sample['next'] != '':
            ...
            sd_rec_next = nusc.get('sample_data', sample_next['data']['INFRASTRUCTURE_LIDAR_V1'])
        ...
        can_bus = _get_can_bus_info(nusc, sample)
        fut_valid_flag = True
        ...
        camera_types = [
             'INFRASTRUCTURE_CAM_V1'
        ]
        for cam in camera_types:
            cam_token = sample['data'][cam]
            camera_data = nusc.get('sample_data', cam_token)
            calibrated_sensor = nusc.get('calibrated_sensor', camera_data['calibrated_sensor_token'])
            camera_intrinsics = calibrated_sensor['camera_intrinsic']
            #cam_path, _, cam_intrinsic = nusc.get_sample_data(cam_token)
            cam_info = obtain_sensor2top(nusc, cam_token, l2e_t, l2e_r_mat,
                                         e2g_t, e2g_r_mat, cam)
            cam_info.update(cam_intrinsic=camera_intrinsics)
        ...
        if not test:
            ...
            delta_x = ego_his_trajs[-1, 0] + ego_fut_trajs[0, 0] #moved from above
            delta_y = ego_his_trajs[-1, 1] + ego_fut_trajs[0, 1]
            v0 = np.sqrt(delta_x**2 + delta_y**2)
            Kappa = 0
...
def get_global_sensor_pose(rec, nusc, inverse=False):
    lidar_sample_data = nusc.get('sample_data', rec['data']['INFRASTRUCTURE_LIDAR_V1'])
...
def export_2d_annotation(root_path, info_path, version, mono3d=False):
    ...
    camera_types = [
        'VEHICLE_CAM_FRONT','INFRASTRUCTURE_CAM_V1'
    ]
...
def nuscenes_data_prep(root_path,
    ...
    create_nuscenes_infos(
        root_path, out_dir, None, info_prefix, version=version, max_sweeps=max_sweeps)
