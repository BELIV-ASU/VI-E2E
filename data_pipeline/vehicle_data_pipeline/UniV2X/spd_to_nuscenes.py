...
def generate_calibrated_sensor_json(spd_infos, data_root, version='v1.0-mini', local_root=''):
    ...
    calibrated_sensors = ['VEHICLE_CAM_FRONT','LIDAR_TOP']#,'INFRASTRUCTURE_CAM_V1','INFRASTRUCTURE_LIDAR_V1']

    calibrated_sensor_infos = []
    for sensor_info in sensor_data:
        print("HERE",sensor_info['channel'])
        if sensor_info['channel'] in calibrated_sensors:
            if sensor_info['modality'] == 'camera':
                print(spd_infos[0].keys())
                try:
                    ...
                    #print("HERE AGAIN",spd_infos[0]['other_agent_info_dict']['model_other_agent_inf'].keys())
                    rotation = spd_infos[0]['other_agent_info_dict']['model_other_agent_inf']['cams'][sensor_info['channel']]['sensor2ego_rotation']
                    translation = spd_infos[0]['other_agent_info_dict']['model_other_agent_inf']['cams'][sensor_info['channel']]['sensor2ego_translation']
                    camera_intrinsic = spd_infos[0]['other_agent_info_dict']['model_other_agent_inf']['cams'][sensor_info['channel']]['cam_intrinsic']
                    token = gen_token(sensor_info['channel'], str(translation[0]), str(translation[1]), str(translation[2]))
                    info = {
                        'token': token,
                        'sensor_token': sensor_info['token'],
...
def generate_sample_data_json(spd_infos, data_root, version='v1.0-mini', local_root=''):
    ...
    sensor_data_1 = load_json(osp.join(data_root, version, 'sensor.json'))
    channel_to_token = {item["channel"]: item["token"] for item in sensor_data_1}
    ...
    for calibrated_sensor in calibrated_sensor_data:
        if calibrated_sensor['camera_intrinsic'] is not []:
            ...
            if sensor_token == channel_to_token["LIDAR_TOP"] or sensor_token == channel_to_token["INFRASTRUCTURE_LIDAR_V1"]:
                import pdb; pdb.set_trace()
                ...
                print("DEBIG STATEMNT")
                if sensor_token == channel_to_token["VEHICLE_CAM_FRONT"]:
                    for sample_info in spd_infos:
                        info = {
                            'token': sample_info['token'],
                            'sample_token': sample_info['token'],
                            ...
                            'height': 0,
                            'width': 0,
                            'filename': sample_info['cams']['VEHICLE_CAM_FRONT']['data_path'],
                            'prev': sample_info['prev'],
                            'next': sample_info['next']
                        }
                ...
                elif sensor_token == channel_to_token["INFRASTRUCTURE_CAM_V1"]:
                    for sample_info in spd_infos:
                        info = {
                            'token': sample_info['token'],
                            'sample_token': sample_info['token'],#sample_info['other_agent_info_dict']['model_other_agent_inf']['token'],
                            'ego_pose_token': sample_info['token'],
                            'calibrated_sensor_token': calibrated_sensor_token,
                            'timestamp': sample_info['timestamp'],
                            'fileformat': 'jpg',
                            ...
                            'height': 0,
                            'width': 0,
                            'filename': sample_info['cams']["INFRASTRUCTURE_CAM_V1"]['data_path'],
                            'prev': sample_info['prev'],
                            'next': sample_info['next']
                        }
