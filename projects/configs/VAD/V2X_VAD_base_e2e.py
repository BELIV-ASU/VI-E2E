...
cameras = 6 # keep the camera-embedding table checkpoint-compatible; runtime branches use real camera count
...
model = dict(
    ...
    pts_bbox_head=dict(
        ...
        valid_fut_ts=6,
        # V2X matching is intentionally more tolerant than 1 m because small
        # camera-pose/time residuals otherwise prevent true vehicle/infra pairs
        # from being fused.  Anchor fusion allows a confident infra query to
        # correct the final box center instead of only changing the query feature.
        ...
        loss_cls=dict(
            ...
            alpha=0.25,
            loss_weight=2.0),
        loss_bbox=dict(type='L1Loss', loss_weight=1.0), #0.25
        loss_traj=dict(type='L1Loss', loss_weight=0.2), #0.2
        loss_traj_cls=dict(
            type='FocalLoss',
            use_sigmoid=True,
            gamma=2.0,
            alpha=0.25,
            loss_weight=0.2),
        loss_iou=dict(type='GIoULoss', loss_weight=1.0), #0.0
        loss_map_cls=dict(
            type='FocalLoss',
            use_sigmoid=True,
            gamma=2.0,
        ...
        loss_map_pts=dict(type='PtsL1Loss', loss_weight=0.5),
        loss_map_dir=dict(type='PtsDirCosLoss', loss_weight=0.005),
        loss_plan_reg=dict(type='L1Loss', loss_weight=0.0), #0.0
        loss_plan_bound=dict(type='PlanMapBoundLoss', loss_weight=1.0, dis_thresh=1.0), #1.0
        loss_plan_col=dict(type='PlanCollisionLoss', loss_weight=1.0), #1.0
        loss_plan_dir=dict(type='PlanMapDirectionLoss', loss_weight=0.5)), #0.5
    ...
    train_cfg=dict(pts=dict(
        ...
        assigner=dict(
            ...
            reg_cost=dict(type='BBox3DL1Cost', weight=1.0),  #0.25
            iou_cost=dict(type='IoUCost', weight=1.0), # Fake cost. This is just to make it compatible with DETR head. 0.0
            pc_range=point_cloud_range),
        map_assigner=dict(
            ...
            pts_cost=dict(type='OrderedPtsL1Cost', weight=0.5),
            pc_range=point_cloud_range))),
...
optimizer = dict(
    ...
    paramwise_cfg=dict(
        custom_keys={
            # Backbones learn slowly to preserve pretrained features
            'img_backbone': dict(lr_mult=0.1),
            'img_backbone_infra': dict(lr_mult=0.1),

            'transformer': dict(lr_mult=0.5),
            'pts_bbox_head.transformer': dict(lr_mult=1.0),
            'pts_bbox_head_infra.transformer': dict(lr_mult=1.0),

            # V2X fusion modules learn faster since they are newly introduced
            'agent_fusion_decoder': dict(lr_mult=2.0),
            'cross_agent_fusion': dict(lr_mult=2.0),
            'vi_map_fuser': dict(lr_mult=2.0),
            'vi_motion': dict(lr_mult=2.0),
            'bev_embed_linear': dict(lr_mult=1.0),
            'bev_pos_linear': dict(lr_mult=1.0),
        }
    )
)
...
load_from = '/scratch/jmeng18/vadv2x_only_fusion/epoch_19_bst.pth' #'/scratch/jmeng18/coop_bev.pth'
