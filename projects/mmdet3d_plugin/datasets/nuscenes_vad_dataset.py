...
class LiDARInstanceLines(object):
    ...
    def shift_fixed_num_sampled_points_v2(self):
        ...
        for instance in self.instance_list:
            ...
            if is_poly:
                # Use equidistant sampled points for cyclic shifting.
                # This gives fixed_num candidate starting positions instead of only
                # (num_polygon_vertices - 1), which can be as few as 4 after simplify().
                # Finer granularity eliminates Hungarian-assignment oscillation that
                # causes the zigzag artifact in predicted map element shapes.
                sampled_points = np.array([list(instance.interpolate(distance).coords) for distance in distances]).reshape(-1, 2)
                for shift_right_i in range(self.fixed_num):
                    shift_pts = np.roll(sampled_points, shift_right_i, axis=0)
...
class VectorizedLocalMap(object):
    ...
    def __init__(self,
        ...
        self.MAPS = ['yizhuang02', 'yizhuang06',
                     'yizhuang08', 'yizhuang09','yizhuang10','yizhuang13','yizhuang16']
    ...
    def ped_poly_geoms_to_instances(self, ped_geom):
        ...
        union_segments = union_segments.simplify(1.0, preserve_topology=True)
        max_x = self.patch_size[1] / 2
        max_y = self.patch_size[0] / 2
        # local_patch = box(-max_x + 0.2, -max_y + 0.2, max_x - 0.2, max_y - 0.2)
        local_patch = box(-max_x - 0.2, -max_y - 0.2, max_x + 0.2, max_y + 0.2)
    ...
    def get_ped_crossing_line(self, patch_box, patch_angle, location):
        ...
        for record in records:
            polygon = self.map_explorer[location].map_api.extract_polygon(record['polygon_token'])
            if polygon.is_valid:
                polygon = polygon.simplify(1.0, preserve_topology=True)
                new_polygon = polygon.intersection(patch)
                if not new_polygon.is_empty:
                    new_polygon = affinity.rotate(new_polygon, -patch_angle,
                                                      origin=(patch_x, patch_y), use_radians=False)
...
class VADCustomNuScenesDataset(NuScenesDataset):
    ...
    #     """
    #     Convert sample queue into one single sample with multi-camera support.
        
    #     Creates aligned structure:
    #     - imgs_list: [cam1_t0, cam2_t0, cam1_t1, cam2_t1, ...]
    #     - metas_list: [cam1_meta_t0, cam2_meta_t0, cam1_meta_t1, cam2_meta_t1, ...]
        
    #     So img[i] and metas[i] correspond to the same camera at the same timestep.
    #     """
    #     imgs_list = []
    #     metas_list = []
    ...
    #     prev_pos = None
    #     prev_angle = None
    #     num_cams_per_frame = None
        
    #     for frame_idx, each in enumerate(queue):
    #         # Get images and metas for this timestep
    #         frame_imgs = each['img'].data  # Could be single img or stacked
    #         frame_metas = each['img_metas'].data
            
    #         # Determine if multi-camera
    #         if isinstance(frame_metas, list):
    #             # Multi-camera case
    #             num_cams = len(frame_metas)
    #             if num_cams_per_frame is None:
    #                 num_cams_per_frame = num_cams
                
    #             for cam_idx, cam_meta in enumerate(frame_metas):
    #                 # Get corresponding image
    #                 if frame_imgs.dim() == 4:  # Single image
    #                     img = frame_imgs
    #                 else:  # Multiple images already stacked
    #                     img = frame_imgs[cam_idx] if cam_idx < len(frame_imgs) else frame_imgs
                    
    #                 imgs_list.append(img)
                    
    #                 # Update can_bus for ego motion
    #                 if frame_idx == 0:
    #                     cam_meta['prev_bev'] = False
    #                     if cam_idx == 0:  # Only compute once per frame
    #                         prev_pos = copy.deepcopy(cam_meta['can_bus'][:3])
    #                         prev_angle = copy.deepcopy(cam_meta['can_bus'][-1])
    #                     cam_meta['can_bus'][:3] = 0
    #                     cam_meta['can_bus'][-1] = 0
    #                 else:
    #                     cam_meta['prev_bev'] = True
    #                     tmp_pos = copy.deepcopy(cam_meta['can_bus'][:3])
    #                     tmp_angle = copy.deepcopy(cam_meta['can_bus'][-1])
    #                     cam_meta['can_bus'][:3] -= prev_pos
    #                     cam_meta['can_bus'][-1] -= prev_angle
                        
    #                     if cam_idx == 0:  # Update for next frame
    #                         prev_pos = copy.deepcopy(tmp_pos)
    #                         prev_angle = copy.deepcopy(tmp_angle)
                    
    #                 metas_list.append(cam_meta)
            
    #         else:
    #             # Single-camera case
    #             if num_cams_per_frame is None:
    #                 num_cams_per_frame = 1
                
    #             imgs_list.append(frame_imgs)
                
    #             if frame_idx == 0:
    #                 frame_metas['prev_bev'] = False
    #                 prev_pos = copy.deepcopy(frame_metas['can_bus'][:3])
    #                 prev_angle = copy.deepcopy(frame_metas['can_bus'][-1])
    #                 frame_metas['can_bus'][:3] = 0
    #                 frame_metas['can_bus'][-1] = 0
    #             else:
    #                 frame_metas['prev_bev'] = True
    #                 tmp_pos = copy.deepcopy(frame_metas['can_bus'][:3])
    #                 tmp_angle = copy.deepcopy(frame_metas['can_bus'][-1])
    #                 frame_metas['can_bus'][:3] -= prev_pos
    #                 frame_metas['can_bus'][-1] -= prev_angle
    #                 prev_pos = copy.deepcopy(tmp_pos)
    #                 prev_angle = copy.deepcopy(tmp_angle)
                
    #             metas_list.append(frame_metas)
        
    #     queue[-1]['img'] = DC(torch.stack(imgs_list), cpu_only=False, stack=True)
    #     queue[-1]['img_metas'] = DC(metas_list, cpu_only=True)
        
    #     return queue[-1]

    # def get_lidar2img(self, cam_name, sensor2lidar_rot, sensor2lidar_trans):
    #     """Compute lidar2img transformation matrix."""
    #     # Convert to numpy if needed
    #     sensor2lidar_rot = Quaternion(sensor2lidar_rot).rotation_matrix
    #     sensor2lidar_trans = np.array(sensor2lidar_trans)
        
    #     # Build 4x4 matrix
    #     sensor2lidar = np.eye(4)
    #     sensor2lidar[:3, :3] = sensor2lidar_rot  # Transpose for inverse
    #     sensor2lidar[:3, 3] = sensor2lidar_trans
    #     lidar2img = np.linalg.inv(sensor2lidar)
    ...
    @staticmethod
    def _recompute_ego_cmd(ego_fut_trajs):
        """Recompute ego command from trajectory stored in V2X-Seq lidar frame.

        The PKL stores per-step delta offsets in lidar frame where lidar x is
        ego forward and lidar y is ego left.  Cumulative sum gives absolute
        final position; lateral displacement (lidar y) determines the command.
        """
        end = np.array(ego_fut_trajs).cumsum(axis=0)[-1]  # final position in lidar frame
        if end[1] <= -2:
            return np.array([1, 0, 0], dtype=np.float32)  # Turn Right (ego right = lidar -y)
        elif end[1] >= 2:
            return np.array([0, 1, 0], dtype=np.float32)  # Turn Left  (ego left  = lidar +y)
        else:
            return np.array([0, 0, 1], dtype=np.float32)  # Go Straight
    ...
    def get_data_info(self, index):
        ...
        input_dict = dict(
            ...
            ego_fut_cmd=self._recompute_ego_cmd(info['gt_ego_fut_trajs']),
            ego_lcf_feat=info['gt_ego_lcf_feat'] #             cams=info.get('cams', {}) # Keep cams dict: modification to be mafe in transforms_3d.CustomCollect3DMultiCam
        ...
        if self.modality['use_camera']:
            ...
            for cam_type, cam_info in info['cams'].items():
                ...
                camera2ego[:3, :3] = cam_info["sensor2ego_rotation"] #Quaternion(
    ...
    def _format_bbox(self, results, jsonfile_prefix=None, score_thresh=0): #0.2
    ...
    def evaluate(self,
        ...
        """
        # custom_multi_gpu_test wraps results in {'bbox_results': [...]}
