...
def load_gt(nusc: NuScenes, eval_split: str, box_cls, verbose: bool = False):
    ...
    # V2X-Seq (and other non-NuScenes datasets) use custom scene names that
    # don't appear in the standard NuScenes split lists.  Fall back to all
    # samples in the DB so that GT loading still works.
...
def filter_eval_boxes(nusc: NuScenes,
    ...
    for ind, sample_token in enumerate(eval_boxes.sample_tokens):
        ...
        eval_boxes.boxes[sample_token] = [box for box in eval_boxes[sample_token] if np.linalg.norm(box.ego_translation[:2],2) < max_dist_x[box.__getattribute__(class_field)]]
        #                                  abs(box.ego_translation[0]) < max_dist_x[box.__getattribute__(class_field)] \
        #                                  and abs(box.ego_translation[1]) < max_dist_y[box.__getattribute__(class_field)]]
