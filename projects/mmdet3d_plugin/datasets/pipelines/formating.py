...
class CustomDefaultFormatBundle3D(DefaultFormatBundle3D):
    ...
    def __call__(self, results):
        ...
        # ANIRUDH EDITED THIS ->
        if 'gt_bboxes_3d_infra' in results:
            assert isinstance(results['gt_bboxes_3d_infra'], BaseInstance3DBoxes)
            results['gt_bboxes_3d_infra'] = DC(results['gt_bboxes_3d_infra'], cpu_only=True)

        if 'gt_labels_3d_infra' in results:
            results['gt_labels_3d_infra'] = DC(
                to_tensor(results['gt_labels_3d_infra']), cpu_only=False
            )

        if 'gt_attr_labels_infra' in results:
            results['gt_attr_labels_infra'] = DC(
                to_tensor(results['gt_attr_labels_infra']), cpu_only=False
            )
        # #
