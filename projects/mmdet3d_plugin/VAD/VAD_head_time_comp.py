...
class VADHead(DETRHead):
    ...
    def get_time_delay(self,img_metas):
        ...
        for meta in img_metas:
            ...
            # Signed time offset in seconds: infra timestamp - vehicle timestamp.
            # Positive means the infrastructure image is later than the vehicle image.
            delta_t_gt.append([ts_infra - ts_ego])
        return torch.tensor(delta_t_gt).float() / 1e6
        # delta_t_gt = delta_t_pred.new_tensor(delta_t_gt).float() / 1e6
        ...
        if training_mode:
            ...
            for i in range(len(infra_global_coords_list)):
                ...
                if (use_time_comp and infra_global_vels_list is not None
                    ...
                    dt_value = dt_value.clamp(
                        -self.v2x_time_sync_max_dt, self.v2x_time_sync_max_dt)

                    global_vel = infra_global_vels_list[i].to(
                        device=infra_reference.device, dtype=infra_reference.dtype)
                    if global_vel.numel() > 0 and global_vel.shape[-1] >= 2:
                        ...
                        valid_vel = finite & (speed > self.v2x_time_sync_min_speed)
                        shift = torch.where(
                            valid_vel, vel_xy * dt_value, torch.zeros_like(vel_xy))
