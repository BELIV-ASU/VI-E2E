...
class VADHead(DETRHead):
    ...
    def __init__(self,
        ...
        with open('/scratch/aiyer43/data/V2X-Seq-SPD-New/infrastructure-side/v1.0-trainval/ego_pose.json','r') as f:
            self.ego_pose_infra = json.load(f)
        
        with open('/scratch/aiyer43/data/V2X-Seq-SPD-New/vehicle-side/v1.0-trainval/ego_pose.json','r') as f:
        ...
        self.map_token_projector = nn.Sequential(
            nn.Linear(100, 2000),  # Project token dim
            nn.ReLU(inplace=False)
        )


        self.frameid_to_ts = {}
        with open('/scratch/aiyer43/data/V2X-Seq-SPD-New/vehicle-side/data_info.json', 'r') as f:
            data_info = json.load(f)
        self.frameid_to_ts["ego"] = {entry['frame_id']: int(entry['image_timestamp']) for entry in data_info}

        with open('/scratch/aiyer43/data/V2X-Seq-SPD-New/infrastructure-side/data_info.json', 'r') as f:
            data_info = json.load(f)
    ...
    def query_generation(self, outputs):
        ...
        for lvl in range(hs.shape[0]): # used to get agent coordinates and their confidence
            ...
            tmp_xy = tmp[..., 0:2] + reference[..., 0:2]
            tmp_xy = tmp_xy.sigmoid()
            tmp[..., 0:2] = tmp_xy
    ...
    def forward(self,
        ...
        """Forward function.
        This file is for VI fusion or v only for downstream predictions
        Args:
        ...
        # ------------------------------ Paper Contribution Point 1 (Temporal Compensation): ------------------------------------------------- #
        # Label for Check the synchronization results
        ...
        map_pos_emb = self.norm_map_pos(map_pos_emb.clone() + T_veh_embed_map)
        ...
        if training_mode:
            ...
            agent_query = agent_query.permute(1,0,2)
        # ------------------------------ Paper Contribution Point 1 End (Temporal Compensation): ------------------------------------------------- #
        ...
        outputs_class = self.cls_branches[-1](agent_query.permute(1,0,2))#self.cls_branches(agent_query)
        tmp = self.reg_branches[-1](agent_query.permute(1,0,2))
        reference = init_reference
        ...
        tmp_z = tmp[..., 4:5] + reference[..., 2:3]
        tmp_z = tmp_z.sigmoid()
        tmp[..., 4:5] = tmp_z

        tmp_a = (tmp[..., 0:1] * (self.pc_range[3] -
                            self.pc_range[0]) + self.pc_range[0])

        tmp[..., 0:1] = tmp_a
        tmp_b = (tmp[..., 1:2] * (self.pc_range[4] -
                            self.pc_range[1]) + self.pc_range[1])
        tmp[..., 1:2] = tmp_b
        tmp_c = (tmp[..., 4:5] * (self.pc_range[5] -
        ...
        # ------------------------------------------------ Paper Contribution Point 2: (Masking) -------------------------------------------- #
        # Map masking
        ...
        #  ----------------------------------------------------- Paper Contribution Point 2: (Masking) End --------------------------------------- # 
