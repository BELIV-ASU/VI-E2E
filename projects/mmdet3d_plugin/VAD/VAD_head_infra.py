...
class VADHeadInfra(DETRHead):
    ...
    def query_generation(self, outputs):
        ...
        ego_his_feats, infra_tmp, infra_class, outputs_coords, outputs_classes
    ...
    def forward(self,
        ...
        sample_token_inf = img_metas[0]['filename'][0].split('/')[-1][:-4]
        ...
        ego_his_feats_i, infra_tmp, infra_class, outputs_coords_i, outputs_classes_i = self.query_generation(outputs_i)
        ...
        outputs_coords_bev = []
        outputs_coords = []
        outputs_global_coords = []

        inf_pc_range = [0, -51.2, -5.0, 102.4, 51.2, 3.0]

        for i in range(len(outputs_coords_i)):
            infra_reference = torch.cat([outputs_coords_i[i][...,:2], outputs_coords_i[i][...,4:5]], dim=2)
            tmp = outputs_coords_i[i]
            outputs_class = outputs_classes_i[i]
            ...
            T_inf_t = torch.as_tensor(T_inf, device=infra_coords.device).to(dtype=infra_h.dtype)  # converts numpy -> torch

            global_h = infra_h @ T_inf_t.transpose(-1, -2)
            #global_h = infra_h @ T_inf.T
            ...
            outputs_classes.append(outputs_class)
            outputs_coords.append(tmp)
            outputs_global_coords.append(infra_global_coords)

        # Compute per-layer global velocities by rotating local vx/vy (code_size cols 8-9)
        # with the 2-D rotation block of T_inf (infra-local → global).
        _R2d = torch.as_tensor(
            T_inf[:2, :2],
            device=outputs_coords[0].device,
            dtype=outputs_coords[0].dtype,
        )  # [2, 2]
        outputs_global_vels = []
        for _coord in outputs_coords:
            # coord format: (cx,cy,w,l,cz,h,rot_sine,rot_cosine,vx,vy) → vx/vy at [8:10]
            _local_vel = _coord[0, :, 8:10]          # [Q, 2], vx/vy in infra-local frame
            _global_vel = _local_vel @ _R2d.T        # [Q, 2], vx/vy in global frame
            outputs_global_vels.append(_global_vel)

        global_inf_pc = self.transform_pc_range_aabb(inf_pc_range,T_inf)
        # # VI motion queries
        # # motion SA
        ...
        preds_dicts_infra = {
            "all_cls_scores": outputs_classes,   # [1, B, Q, C]
            "all_bbox_preds": outputs_coords,             # [1, B, Q, code_size]
        }

    
        return {
            "preds_dicts": preds_dicts_infra,
            "queries": (ego_query_i, agent_query_i, ego_pos_emb_i, agent_pos_emb_i, agent_mask_i, map_query_i, map_pos_emb_i, map_mask_i,
                        ego_his_feats_i, ca_motion_query, infra_global_coords, outputs_classes, infra_coords, infra_tmp, outputs_global_coords, outputs_coords,
                        outputs_global_vels),  # 17th element: per-layer global velocities for time compensation
            "global_pc_range": global_inf_pc,
        }
