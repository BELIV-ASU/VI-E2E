    ...
    def forward(self,
        ...
        if training_mode:
            ...
            for i in range(len(infra_global_coords_list)):
                infra_reference = infra_global_coords_list[i]
                ...
                T_global_to_local_t = torch.as_tensor(T_global_to_local,device=infra_global_h.device,dtype=infra_global_h.dtype)
                infra_local_h = infra_global_h @ T_global_to_local_t.transpose(-1, -2)
                #infra_local_h = infra_global_h @ T_global_to_local.T
                infra_reference = infra_local_h[:, :3].unsqueeze(0).to(infra_global_h.device)

            
                infra_tmps[i][..., 0:1] = (infra_reference[..., 0:1] - self.pc_range[0]) / (self.pc_range[3] - self.pc_range[0])
                infra_tmps[i][..., 1:2] = (infra_reference[..., 1:2] - self.pc_range[1]) / (self.pc_range[4] - self.pc_range[1])
                infra_tmps[i][..., 4:5] = (infra_reference[..., 2:3] - self.pc_range[2]) / (self.pc_range[5] - self.pc_range[2])
                infra_class = infra_classes[i]
                infra_tmps[i][..., 0:2] = inverse_sigmoid(infra_tmps[i][..., 0:2].clone())
                infra_tmps[i][..., 4:5] = inverse_sigmoid(infra_tmps[i][..., 4:5].clone())
                infra_tmp_list.append(infra_tmps[i])
            infra_tmp = infra_tmps[-1]
            # Agent Query fusion
            ...
            veh_prob_norm = F.normalize(veh_prob, p=2, dim=-1, eps=1e-6)
            inf_prob_norm = F.normalize(inf_prob, p=2, dim=-1, eps=1e-6)
            semantic_sim = torch.matmul(veh_prob_norm, inf_prob_norm.t()).clamp(0.0, 1.0)
            semantic_cost = 1.0 - semantic_sim
            ...
            # Category mismatch is only penalized when both branches are confident.
            # Low-confidence semantic predictions are ignored rather than used to

            
            cost = dist2.clone()
            cost = cost.masked_fill(~same_cls, 1e6)

            # 4) gate far pairs (optional but recommended)
            cost = cost.masked_fill(dist2 > dist_thresh**2, 1e6)

            # 5) Hungarian (SciPy wants CPU numpy)
            ...
            #matched_dist = torch.norm(veh_xy[row_idx] - inf_xy[col_idx], dim=-1)
            #valid_dist = matched_dist < dist_thresh
           
            #infra_ref = infra_reference.squeeze(0) 
            #inside_infra = (infra_ref[:, 0] >= self.pc_range[0]) & (infra_ref[:, 0] <= self.pc_range[3]) & (infra_ref[:, 1] >= self.pc_range[1]) & (infra_ref[:, 1] <= self.pc_range[4])
            #valid_inside = inside_infra[col_idx]
           
            #valid = valid_sim & valid_dist & valid_inside
            ...
            fusion_tmp = veh_tmp[:, veh_idx, :] #0.5*veh_tmp[:, veh_idx, :] + 0.5*infra_tmp[:,infra_idx,:]
            fusion_class = veh_class[:, veh_idx, :]
            fusion_pos_emb = 0.5*agent_pos_emb[:,veh_idx,:] + 0.5*agent_pos_emb_i[:,infra_idx,:]
            
            for i in range(Ni):
                if i not in infra_idx and confidence[:, i] > 0.85: # to remove concatenation change this to 1.0
