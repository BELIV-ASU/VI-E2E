...
class VADHead(DETRHead):
    ...
    def __init__(self,
        ...
        with open('/scratch/jmeng18/V2X-Seq-SPD-New/vehicle-side/v1.0-trainval/ego_pose.json','r') as f:
        ...
        self.frameid_to_ts = {}
        with open('/scratch/jmeng18/V2X-Seq-SPD-New/vehicle-side/data_info.json', 'r') as f:
            data_info = json.load(f)
        self.frameid_to_ts["ego"] = {entry['frame_id']: int(entry['image_timestamp']) for entry in data_info}

        with open('/scratch/jmeng18/V2X-Seq-SPD-New/infrastructure-side/data_info.json', 'r') as f:
            data_info = json.load(f)
    ...
    def query_generation(self, outputs):
        ...
        ego_his_feats, veh_tmp, veh_class
    ...
    def get_time_delay(self,img_metas):
        ...
        for meta in img_metas:
            ego_fname = os.path.basename(meta['filename'][0])
            infra_fname = os.path.basename(meta['filename'][1])
            ego_frame_id = os.path.splitext(ego_fname)[0]
            infra_frame_id = os.path.splitext(infra_fname)[0]
            ...
            delta_t_gt.append([abs(ts_ego - ts_infra)])
        return torch.tensor(delta_t_gt).float() / 1e6
        # delta_t_gt = delta_t_pred.new_tensor(delta_t_gt).float() / 1e6  # convert to ms
    ...
    def forward(self,
        ...
        T_veh = self.get_transform(self.ego_pose_vehicle, sample_token_veh)
        ...
        #ego
        ego_query, agent_query, ego_pos_emb, agent_pos_emb, agent_mask, map_query, map_pos_emb, map_mask,\
        ego_his_feats, veh_tmp, veh_class = self.query_generation(outputs)

        if training_mode:
            #infra
            ego_query_i, agent_query_i, ego_pos_emb_i, agent_pos_emb_i, agent_mask_i, map_query_i, map_pos_emb_i, map_mask_i,\
            ego_his_feats_i, ca_motion_query_i, infra_global_coords, infra_classes, infra_coords, infra_tmp = infra_queries

        #prepare ego position in global coordinate system
        ego_pos = torch.zeros((1, 1, 2), device=agent_query.device)
        ...
        T_veh_embed_agent = self.agent_transform_mlp(T_veh_tensor.expand(agent_query.shape[0], agent_query.shape[1], -1))  # [B, N, D]
        ...
        if training_mode:
            ...
            #agent_query_fusion = agent_query[:, veh_idx, :].permute(1, 0, 2)
            ...
            #veh_reference = torch.cat([veh_tmp[...,:2], veh_tmp[...,4:5]], dim=2)  #[1,900,3]
            #veh_reference[..., 0:1] = (veh_reference[..., 0:1] * (self.pc_range[3] - self.pc_range[0]) + self.pc_range[0])
            #veh_reference[..., 1:2] = (veh_reference[..., 1:2] * (self.pc_range[4] - self.pc_range[1]) + self.pc_range[1])
            #veh_reference[..., 4:5] = (veh_reference[..., 4:5] * (self.pc_range[5] - self.pc_range[2]) + self.pc_range[2])
            #veh_reference = inverse_sigmoid(veh_reference)
            veh_tmp[..., 0:2] = inverse_sigmoid(veh_tmp[..., 0:2].clone())
            veh_tmp[..., 4:5] = inverse_sigmoid(veh_tmp[..., 4:5].clone())
            fusion_tmp = veh_tmp[:, veh_idx, :]
            fusion_class = veh_class[:, veh_idx, :]

            infra_reference = infra_global_coords
            T_veh_t = torch.from_numpy(T_veh)   # convert to FP32 tensor
            T_global_to_local = torch.linalg.inv(T_veh_t)  # robust batched inverse
            ones = torch.ones((infra_reference.size(0), 1))
            infra_global_h = torch.cat([infra_reference, ones], dim=1)   # [900, 4]
            infra_local_h = infra_global_h @ T_global_to_local.T
            infra_reference = infra_local_h[:, :3].unsqueeze(0).cuda()
            
            infra_tmp[..., 0:1] = (infra_reference[..., 0:1] - self.pc_range[0]) / (self.pc_range[3] - self.pc_range[0])
            infra_tmp[..., 1:2] = (infra_reference[..., 1:2] - self.pc_range[1]) / (self.pc_range[4] - self.pc_range[1])
            infra_tmp[..., 4:5] = (infra_reference[..., 2:3] - self.pc_range[2]) / (self.pc_range[5] - self.pc_range[2])
            infra_class = infra_classes[-1]
            infra_tmp[..., 0:2] = inverse_sigmoid(infra_tmp[..., 0:2].clone())
            infra_tmp[..., 4:5] = inverse_sigmoid(infra_tmp[..., 4:5].clone())
            
            for i in range(Ni):
                if i not in infra_idx and confidence[:, i] > 0.8:
            ...
            #infra_unmatched_reference[...,2] = infra_coords[infra_unmatched_idx,2].unsqueeze(0).cuda()
            
            
            #infra_unmatched_reference = sigmoid(infra_unmatched_reference).cuda()
                    
            cur_len = agent_query_fusion.shape[1] + agent_query_i_unmatched.shape[1]
            
            if cur_len < Nv:
                #veh_unmatched_idx = torch.tensor(random.sample(list(set(range(Nv)) - set(veh_idx.tolist())), Nv - cur_len))
                veh_unmatched_idx = torch.tensor(list(set(range(Nv)) - set(veh_idx.tolist())))
                agent_query_v_unmatched = agent_query[:,veh_unmatched_idx,:]
                veh_unmatched_tmp = veh_tmp[:, veh_unmatched_idx, :]
                veh_unmatched_class = veh_class[:, veh_unmatched_idx, :]
                agent_query = torch.cat([agent_query_fusion, agent_query_i_unmatched, agent_query_v_unmatched], dim=1)
                agent_pos = torch.cat([agent_pos_emb[:,veh_idx,:], agent_pos_emb_i[:,infra_unmatched_idx,:], agent_pos_emb[:,veh_unmatched_idx,:]], dim=1)
                tmp = torch.cat([fusion_tmp, infra_unmatched_tmp, veh_unmatched_tmp], dim=1)
                outputs_class = torch.cat([fusion_class, infra_unmatched_class, veh_unmatched_class], dim=1)
                
                #reference = inverse_sigmoid(reference)
                
            else:
                agent_query = torch.cat([agent_query_fusion, agent_query_i_unmatched], dim=1)  # [1, 900, 3]
                agent_pos = torch.cat([agent_pos_emb[:,veh_idx,:], agent_pos_emb_i[:,infra_unmatched_idx,:]], dim=1)
                tmp = torch.cat([fusion_tmp, infra_unmatched_tmp], dim=1)
                outputs_class = torch.cat([fusion_class, infra_unmatched_class], dim=1)
                #reference = inverse_sigmoid(reference)


            # Anchor boxes fusion
            
            #agent_query = self.vi_agent_fuser(
                #query=agent_query.permute(1, 0, 2),
                #key=agent_query_i.permute(1,0,2), # TODO: [A_i,B,D] -> [M_i,B,D] M = A * fut_mode
                #value=agent_query_i.permute(1,0,2),
                #query_pos= None, #veh_agent_pos_embed.permute(1, 0, 2),
                #key_pos=None, #infra_agent_pos_embed.permute(1, 0, 2),
                #key_padding_mask=agent_mask_i.permute(1,0).bool()
            #) #[A_v,B,D]
            ...
            map_query = self.vi_map_fuser(
                ...
                query_pos= None, #veh_map_pos_embed.permute(1, 0, 2),
                key_pos= None, #infra_map_pos_embed.permute(1, 0, 2),
        ...
        #decoder_out = self.v2x_decoder(agent_queries=agent_query.permute(1,0,2),bev_queries=bev_embed.permute(1,0,2))
        #reference = inverse_sigmoid(reference)
        hs = hs.permute(0,2,1,3)
        ...
        #outputs_coord = tmp
        #outputs_classes.append(outputs_class)
        #outputs_coords.append(outputs_coord) #What coordinate system?

        reference = map_inter_references[-1]#map_init_reference
        reference = inverse_sigmoid(reference)
        
        target_len = self.map_num_vec * self.map_num_pts_per_vec  # e.g., 2000
        B, P, D = map_query.size()             # [B, P1, D]

        #  TODO: the map query comes from hidden state in source code
        map_query_tmp = map_query.unsqueeze(2).repeat(1, 1, self.map_num_pts_per_vec, 1) #[B,P,pts,D]
        map_query_tmp =  map_query_tmp.view(B, target_len, D) #[B,2000,D]
 
        map_outputs_class = self.map_cls_branches[-1]((
                map_query_tmp.view(1,self.map_num_vec, self.map_num_pts_per_vec,-1).mean(2)
            ))
        ...
        ### comment it out and bring the self.select_and_mask_map back,  while including the masking module
        A = motion_coords.shape[1]
        map_query_masked = map_query.repeat(A,1,1)
        map_query_ego = map_query.clone()
        map_mask_masked = map_mask.repeat(A,1)
        map_mask_ego = map_mask.clone()
        ###
        ...
        #map_query_masked, map_pos_masked, map_mask_masked, map_query_ego, map_pos_ego, map_mask_ego  = self.select_and_mask_map(
            ...
            #road_edge_idx=2,  
            #pe_normalization=False, # if you want [B, P1, pts, D]
            #use_fix_pad=True) #[B*M,P_veh,D]
