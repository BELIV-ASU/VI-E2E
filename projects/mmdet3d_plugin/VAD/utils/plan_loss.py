...
class PlanCollisionLoss(nn.Module):
    ...
    def forward(self,
        ...
        loss_bbox = self.loss_weight * plan_col_loss(ego_fut_preds, agent_preds,
                                                           ...
                                                           dis_thresh=self.agent_search_radius,
                                                           reduction=reduction, avg_factor=avg_factor)
        return loss_bbox
...
def plan_col_loss(
    ...
    # filter agents beyond search radius from ego trajectory
    ...
    # Find the single nearest agent in 2D for each timestep so x and y
    # distances are measured to the same agent (prevents pairing an agent
    # that is merely close in x with a different agent that is close in y).
    nearest_agent_idxs = torch.argmin(x_dist + y_dist, dim=1).tolist()
    batch_idxs = [[i] for i in range(y_dist.shape[0])]
    ts_idxs = [[i for i in range(y_dist.shape[-1])] for j in range(y_dist.shape[0])]

    # [B, fut_ts]
    x_min_dist = x_dist[batch_idxs, nearest_agent_idxs, ts_idxs]
    y_min_dist = y_dist[batch_idxs, nearest_agent_idxs, ts_idxs]
    x_loss = x_min_dist
