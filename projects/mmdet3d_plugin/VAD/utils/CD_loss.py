...
@LOSSES.register_module()
class PtsCurvatureLoss(nn.Module):
    """Penalizes deviation of predicted curvature from GT curvature.

    For each matched line, computes the second-order finite difference
    (d2 = pts[i+1] - 2*pts[i] + pts[i-1]) on both pred and GT, then
    takes L1 of the residual.  This is zero for perfectly matched
    curvature profiles and nonzero for zigzag deviations from GT shape.
    """

    def __init__(self, loss_weight=1.0):
        super(PtsCurvatureLoss, self).__init__()
        self.loss_weight = loss_weight

    def forward(self, pred, target, avg_factor=None, **kwargs):
        """
        Args:
            pred (Tensor): shape [N, num_pts, 2], normalized [0,1] coords
            target (Tensor): shape [N, num_pts, 2], normalized [0,1] coords
            avg_factor (int): number of positive samples for normalisation
        """
        if pred.shape[0] == 0 or pred.shape[1] < 3:
            return pred.sum() * 0
        d2_pred   = pred[:, 2:, :]   - 2 * pred[:, 1:-1, :]   + pred[:, :-2, :]
        d2_target = target[:, 2:, :] - 2 * target[:, 1:-1, :] + target[:, :-2, :]
        loss = (d2_pred - d2_target).abs()
